...
const MENU = [
  { yol: "/uygulama", ad: "Öğren", ikon: "🏠" },
  { yol: "/uygulama/ligler", ad: "Ligler", ikon: "🏆" },
  { yol: "/uygulama/gorevler", ad: "Görevler", ikon: "📋" },
  { yol: "/uygulama/istatistik", ad: "İstatistik", ikon: "📊" },
  { yol: "/uygulama/oyunlar", ad: "Oyunlar", ikon: "🎮" },
  { yol: "/uygulama/profil", ad: "Profil", ikon: "🐱" },
  { yol: "/uygulama/ayarlar", ad: "Daha Fazla", ikon: "⋯" },
];
...
export default function Kabuk({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!yukleniyor && kullanici && !profil) router.replace("/uygulama/kayit/google");
  ...
  const aktifMi = (m: string) => (m === "/uygulama" ? yol === m : yol.startsWith(m));

  return (
    <div className="bk">
      <div className="bk-mobil-ust">
        <Link href="/uygulama" className="bk-logo" style={{ fontSize: 24, padding: 0 }}>
          bilkie
        </Link>
      ...
      <div className="bk-kabuk">
        <aside className="bk-kenar">
          <Link href="/uygulama" className="bk-logo">bilkie</Link>
          {MENU.map((m) => (
          ...
          <div style={{ flex: 1 }} />
          {/* Misafir yok: /uygulama'ya yalnızca girişli kullanıcı gelebiliyor (bkz. Kapi.tsx) */}
        ...
        <div className="bk-govde">
          ...
          <aside className="bk-ray">
            ...
            {!yol.startsWith("/uygulama/ligler") && <LigKarti xp={ust?.xp ?? null} />}
            {!yol.startsWith("/uygulama/gorevler") && <GorevKarti gorevler={gorevler} />}
          </aside>
        </div>
      </div>
...
function Sayaclar({ ust, kisa = false }: { ust: UstBilgi | null; kisa?: boolean }) {
  ...
  const satir = (
    <>
      <Link href="/uygulama/seri" className={`bk-sayac seri ${sonuk ? "sonuk" : ""}`} title="Seri">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/uygulama/seriicon.svg" alt="" />
        <b>{deger(ust?.seri)}</b>
      </Link>
      <Link href="/uygulama/ligler" className={`bk-sayac puan ${sonuk ? "sonuk" : ""}`} title="Toplam XP">
        {/* eslint-disable-next-line @next/next/no-img-element */}
  ...
  return (
    <div className="bk-kart">
      <div className="bk-kart-ust">
        <h3>{lig.ad} Ligi</h3>
        <Link href="/uygulama/ligler">LİGİ GÖSTER</Link>
      </div>
  ...
  return (
    <div className="bk-kart">
      <div className="bk-kart-ust">
        ...
        <Link href="/uygulama/gorevler">TÜMÜNÜ GÖSTER</Link>
      </div>
